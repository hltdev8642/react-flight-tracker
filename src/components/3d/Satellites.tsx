...
function updateSatellitePositions(satellitePositions: { x: number; y: number; z: number }[], SatelliteData: { TLE_LINE1: string; TLE_LINE2: string; }[], altitudeFactor: number) {
    ...
    SatelliteData?.forEach((gp: { TLE_LINE1: string; TLE_LINE2: string; }) => {
        ...
        if (positionEci) {
            ...
            const cartesian = convertToCartesian(
                ...
                EARTH_RADIUS + positionGd.height * 1000
                * reductionFactor * altitudeFactor,
            );
...
export default function Satellites() {
    ...
    updateSatellitePositions(satellitePositions, SatelliteData, altitudeFactor);
    ...
    }, [SatelliteData, altitudeFactor, satellitePositions]);
    useEffect((() => {
        if (satellitePositions) { // Set positions

        }
    }), [satellitePositions, camera]);
