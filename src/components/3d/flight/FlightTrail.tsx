...
export default function FlightTrail() {
  ...
  const annotationsEnabled = useRecoilValue(
    miscellaneousOptionsState,
  ).enableAnnotations;
  ...
  let points =
    data?.trail &&
    data?.trail?.length > 3 &&
    data?.trail?.map((trailPoint) => {
      const cartesian = convertToCartesian(
        trailPoint.lat,
        trailPoint.lng,
        EARTH_RADIUS + trailPoint.alt * reductionFactor * altitudeFactor,
      );
      return new Vector3(cartesian.x, cartesian.y, cartesian.z);
    });
  if (points) {
    const cartesian = convertToCartesian(
      selectedFlight?.trailEntity.lat || 0,
      selectedFlight?.trailEntity.lng || 0,
      EARTH_RADIUS +
        (selectedFlight?.trailEntity?.alt || 0) *
          reductionFactor *
          altitudeFactor,
    );
  ...
  const destination = convertToCartesian(
    data?.airport?.destination?.position.latitude || 0,
    data?.airport?.destination?.position.longitude || 0,
    EARTH_RADIUS +
      (data?.airport?.destination?.position.altitude || 0) *
        reductionFactor *
        altitudeFactor +
      0.001,
  );
  const origin = convertToCartesian(
    data?.airport?.origin?.position?.latitude || 0,
    data?.airport?.origin?.position?.longitude || 0,
    EARTH_RADIUS +
      (data?.airport?.origin?.position?.altitude || 0) *
        reductionFactor *
        altitudeFactor +
      0.001,
  );
  const [originAnnotationVisible, setOriginAnnotationVisible] = useState(false);
  const [destinationAnnotationVisible, setDestinationAnnotationVisible] =
    useState(false);
  return (
    <>
      <Line
      ...
      {data && data.airport?.destination && data.airport?.origin && (
        <>
          {points && points.length > 1 && (
            <Line
              ...
              gapSize={0.01}
              dashSize={0.01}
              points={listInterpolatedGeoCoordinates(
                {
                ...
                const cartesian = convertToCartesian(
                  ...
                  point.longitude,
                  EARTH_RADIUS +
                    point.altitude * reductionFactor * altitudeFactor,
                );
                return new Vector3(cartesian.x, cartesian.y, cartesian.z);
              })}
            />
          ...
          <Sphere
            args={[EARTH_RADIUS / 100, 10, 10]}
            position={new Vector3(destination.x, destination.y, destination.z)}
            onClick={() => {
