...
export function calculateJulianDate(date: Date) {
    return calculateJulianDayNumber(date) +
        +(date.getUTCHours() - 12) / 24 + date.getUTCMinutes() / 1440 + date.getUTCSeconds() / 86400 + (date.getUTCMilliseconds() / 1000) / 86400000000
}
...
export function calculateGreenwichMeanSiderealTime(date: Date) {
    ...
    const t = d_tt / 36525
    return ((280.46061837 + 360.98564736629 * d_tt + (0.000387933 * t * t) - (t * t * t / 38710000)) % 360)
}
...
export function calculateOrbitalElements(primaryOrbitalElements: PrimaryOrbitalElements, date: Date): CalculatedPrimaryOrbitalElements {
 ...
 * @param e The eccentricity
 */
export function calculateTrueAnomalyAndDistance(a: number, E: number, e: number) {
 ...
 *
 */
export function calculatePosition(primaryOrbitalElements: PrimaryOrbitalElements, date: Date) {
    const {N, i, w, a, e, M} = calculateOrbitalElements(primaryOrbitalElements, date)
 ...
 * @param zg
 */
export function calculateEquatorialCoordinates(xg: number, yg: number, zg: number) {
 ...
 * @param zeq
 */
export function calculateRightAscensionAndDeclination(xeq: number, yeq: number, zeq: number) {
