...
export function interpolateGeoCoordinates(
    ...
    const d = Math.acos(Math.sin(
            toRadians(start.latitude)
        )
        * Math.sin(toRadians(end.latitude))
    ...
    const x = A * Math.cos(toRadians(start.latitude)) * Math.cos(toRadians(start.longitude))
        + B * Math.cos(toRadians(end.latitude)) * Math.cos(toRadians(end.longitude));
    const y = A * Math.cos(toRadians(start.latitude)) * Math.sin(toRadians(start.longitude))
        + B * Math.cos(toRadians(end.latitude)) * Math.sin(toRadians(end.longitude));
    const z = A * Math.sin(toRadians(start.latitude)) + B * Math.sin(toRadians(end.latitude));
